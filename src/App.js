...
// OpenAI TTS default voices (all supported)
...
function App() {
  ...
  const [magicWandExplanation, setMagicWandExplanation] = useState('');
  ...
  const handlePdfUpload = async (event) => {
    ...
    try {
      ...
      setPdfText('');
      alert('Could not extract text from PDF. Try another file.');
    }
  };
  ...
  const loadPromptTemplate = async () => {
    try {
      ...
      const pdfContent = pdfText ? pdfText : 'None provided.';
      
      return template
        .replace('{num_words}', numWords)
        .replace('{instructions}', instructions)
        .replace('{pdf_content}', pdfContent);
    } catch (error) {
  ...
  const handleMagicWand = async () => {
    if (!narrationText) {
      alert('Generate narration text first so the AI can pick the best voice.');
      return;
    }
    setIsSelectingVoice(true);
    setMagicWandExplanation('');
    try {
      const [promptRes, descRes] = await Promise.all([
        fetch('/prompt_voice.txt'),
        fetch('/voice_description.json')
      ]);
      const promptTemplate = await promptRes.text();
      const voiceDescriptions = await descRes.json();
      const voiceListText = Object.entries(voiceDescriptions)
        .map(([name, desc]) => `${name}: ${desc}`)
        .join('\n');

      const filledPrompt = promptTemplate
        .replace('{voice_descriptions}', voiceListText)
        .replace('{instructions}', instructions || '(none)')
        .replace('{pdf_content}', pdfText || '(none)')
        .replace('{narration_text}', narrationText.slice(0, 2000));

      const response = await openai.responses.create({
        model: process.env.REACT_APP_MODEL || 'gpt-4o',
        input: [{ role: 'user', content: [{ type: 'input_text', text: filledPrompt }] }]
      });
      ...
      if (!jsonMatch) {
        setMagicWandExplanation('Could not parse AI response. Try again.');
        return;
      }
      let parsed;
      try {
        ...
        setMagicWandExplanation('Invalid JSON from AI. Try again.');
        return;
      }
      const voice = (parsed.voice || '').toLowerCase();
      if (OPENAI_VOICES.includes(voice)) {
        setSelectedVoice(voice);
      }
      setMagicWandExplanation(parsed.explanation || 'Voice selected.');
    } catch (err) {
      console.error('Magic wand error:', err);
      setMagicWandExplanation('Error: ' + (err.message || String(err)));
    } finally {
        ...
        {activeTab === 'narration' && (
          <div className="narration-tab">
            ...
            <div className="pdf-section">
              ...
              />
              {pdfFile && <p className="progress-message">PDF loaded: {pdfFile.name}</p>}
            </div>

            <div className="narration-section">
              <button
                className={`btn-heart ${isGeneratingText ? 'heartbeat' : ''}`}
                onClick={createNarrationText}
                disabled={isGeneratingText || !frames.length || !instructions}
                title="Create Narration Text"
              >
                <span className="heart-icon">❤️</span> {isGeneratingText ? 'Generating...' : 'Create Narration Text'}
              </button>
              ...
              {narrationText && (
                <div className="audio-section">
                  <div className="voice-row">
                    ...
                    <button
                      type="button"
                      className={`btn-heart ${isSelectingVoice ? 'heartbeat' : ''}`}
                      onClick={handleMagicWand}
                      disabled={isSelectingVoice}
                      title="Auto-select best voice"
                    >
                      <span className="heart-icon">🪄</span> {isSelectingVoice ? 'Choosing...' : 'Magic Wand'}
                    </button>
                  </div>
                  {magicWandExplanation && (
                    <div className="magic-wand-explanation" aria-live="polite">
                      {magicWandExplanation}
                    </div>
                  )}
                  <button
                    className={`btn-heart ${isGeneratingAudio ? 'heartbeat' : ''}`}
                    onClick={createNarrationAudio}
                    disabled={isGeneratingAudio || !narrationText}
                    title="Create Narration Audio"
                  >
                    <span className="heart-icon">❤️</span> {isGeneratingAudio ? 'Generating...' : 'Create Narration Audio'}
                  </button>
        ...
        {activeTab === 'mixing' && (
          <div className="mixing-tab editor-layout">
            ...
            <div className="editor-actions">
              <button
                className={`btn-heart ${isMerging ? 'heartbeat' : ''}`}
                onClick={mergeVideoWithAudio}
                disabled={isMerging || !videoFile || !narrationAudioUrl}
                title="Create Video"
              >
                <span className="heart-icon">❤️</span> {isMerging ? 'Creating Video...' : 'Create Video'}
              </button>
            </div>
          </div>
        )}
